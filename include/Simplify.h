...
struct vec3f
{
	...
	//inline 內勘函數
	//函數中的程式碼或處理較短，並且常呼叫使用的話，可以給他加上個inline
...
namespace Simplify
{
	...
	//
	// target_count  : target nr. of triangles 目標要剩下多少triangles
	...
	void update_mesh(int iteration)
	{
	    //如果iteration > 0 才會進入 , 但這邊目前只有 iteration 是0的時候 call update_mesh
	    //所以永遠不會進入到裡面的if
	...
	void load_obj(const char* filename){
		...
		if ((fn = fopen(filename, "rb")) == NULL) //讀取二進位檔(obj檔)
		{
			printf ( "File %s not found!\n" ,filename );
			return;
		}
		char line[1000];
		memset ( line,0,1000 ); //第1000個設置為0
		int vertex_cnt = 0;
		while(fgets( line, 1000, fn ) != NULL) //一行一行讀取obj檔的參數 (讀出來的值存到line[] , 最多讀1000字, Obj檔)
		{
			Vertex v;  //line271 struct Vertex { vec3f p;int tstart,tcount;SymetricMatrix q;int border;};
			if ( line[0] == 'v' )  //讀obj檔 v -0.582493 0.251786 0.420349
			{
				if ( line[1] == ' ' )
				if(sscanf(line,"v %lf %lf %lf", //把 line 每一行從obj 讀的x.y.z 三個數值 放到 Vertex -> vec3f -> x or y or z
					&v.p.x,	&v.p.y,	&v.p.z)==3)
				{
					vertices.push_back(v); //新增Vertex至 vector 的尾端，必要時會進行記憶體配置
				}
			}
			int integers[9];
			if ( line[0] == 'f' ) //讀obj檔 f 19670//19670 20464//20464 20463//20463 (19670 代表第19760個v 以及 第19760個vn
			{
				Triangle t; //line 272 struct Triangle { int v[3];double err[4];int deleted,dirty;vec3f n; };
				bool tri_ok = false;

				if(sscanf(line,"f %d %d %d",
					&integers[0],&integers[1],&integers[2])==3) //判斷這個面是不是合法的三角形(有三點) , if else 主要是判斷每個點值的寫法
				{
					tri_ok = true;
				}else
				if(sscanf(line,"f %d// %d// %d//",
				...
				if(sscanf(line,"f %d//%d %d//%d %d//%d",        //判斷這個面是不是合法的三角形(有三點) , if else 主要是判斷每個點值的寫法
					&integers[0],&integers[3],                  //f 19670(integers[0])//19670(integers[3])
                                                                //20464(integers[1])//20464(integers[4])
                                                                //20463(integers[2])//20463(integers[5])
				...
				else
				{
					...
					printf("%s\n",line);
					//while (1) 迴圈是很好用的迴圈，很多軟體設計都需要用到類似 while (1) 的永真迴圈。因為我們通常希望只有在使用者決定關閉軟體時，才會結束程式的執行，
					//因此利用 while (1) 迴圈，另佐以一個結束機制，就可以輕易用 break 跳出 while (1) 迴圈了
					while(1);
				}
				if ( tri_ok )
				{
				    //line 272 struct Triangle { int v[3];double err[4];int deleted,dirty;vec3f n; };
				    //f 19670(integers[0])//19670 20464(integers[1])//20464 20463(integers[2])//20463
